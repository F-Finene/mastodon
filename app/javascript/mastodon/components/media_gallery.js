...
class Item extends React.PureComponent {
  ...
  render () {
    ...
    if ((size === 5 && index === 4) || (size === 7 && index === 6) || (size === 9 && index === 8) || (size === 11 && index === 10)){
      width = 100;
    }
...
style.height+30;
    const size     = media.take(12).size;
    const uncached = media.every(attachment => attachment.get('type') === 'unknown');

    if (standalone && this.isFullSizeEligible()) {
      children = <Item standalone autoplay={autoplay} onClick={this.handleClick} attachment={media.get(0)} displayWidth={width} visible={visible} />;
    } else {
      children = media.take(12).map((attachment, i) => <Item key={attachment.get('id')} autoplay={autoplay} onClick={this.handleClick} attachment={attachment} index={i} size={size} displayWidth={width} visible={visible || uncached} />);
    }
